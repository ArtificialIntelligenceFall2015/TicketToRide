...
public class PathHandler {
	...
	public static void determinePathClose(DestinationCard cards,
			List<Path> paths) {
		int size = Constants.city.values().length;
		matrix = new boolean[size][size];
		for (Path path : paths) {
			matrix[cityArray.indexOf(path.getCity1())][cityArray.indexOf(path
					.getCity2())] = true;
			matrix[cityArray.indexOf(path.getCity2())][cityArray.indexOf(path
					.getCity1())] = true;
		}
		cards.setCompleted(pathClose(cards.getCity1(), cards.getCity2(),
				new ArrayList<city>()));
	}
		...
		if (start == end)
			...
			for (int i = 0; i < size; i++) {
				if (matrix[index][i] && !closer.contains(cityArray.get(i))) {
					...
					close = close
							|| pathClose(cityArray.get(i), end, closerCopy);
				}
			}
		}
		return close;
