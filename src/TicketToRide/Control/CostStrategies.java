...
public class CostStrategies {
	...
	/**
	 * do i own the path from x->y? (if someone else owns it, it isnt a valid path)
	 * if own by player, return cost 0, no other calculation
	 * @param player
	 * @param path
	 * @return
	 */
	...
	// what is the point value on the destination card to complete the entire route? (is it worth the effort? conservative players choose small point routes, risky players choose big point routes)
	
	/**
	 * what is my rank on the scoreboard? 
	 * (if im in the lead, 
	 * i could be conservative and take small routes 
	 * that keep me there and avoid big routes that might deduct points)
	 * (if im in last place, i can afford to be risky and go for big route cards to gain a lead)
	 * cost/rank, if less than 1, rank/cost
	 * @param player
	 * @param path
	 * @return
	 */
	...
	/**
	 * does this path connect 2 paths owned by another player/players? 
	 * (can i block someone from completing a route? am i in a rush to complete this route before someone cuts me off?)
	 * @param path
	 * @return
	 */
	...
	/**
	 * what is point value associated with claiming the route? 
	 * (15 points for a 6 car route vs only 2 points for a 2 car route…)
	 * @param path
	 * @return
	 */
	...
	/**
	 * what train cards are currently in my hand?	
	 * what train cards are face up?
	 * combine the total number of claim able cards and make the difference between the cost of path
	 * the more close to the cost of path, it return lower cost strategies 
	 * @param player
	 * @param path
	 * @return
	 */
	...
	/**what is the probability of the train cards i need to complete this route/path being in the deck? 
	+ keep track of total cards of every color starting at beginning of game
	- cards i have in my hand (both from blind draw and face up)
	- cards face up
	- cards other players have picked up from face up (can’t know what they picked up from blind draw)
	- cards other players have spent on routes/cards ive spent on routes (discard pile contents)
	**/
	
	/**
	 * can one or more of the routes on these destination cards be combined? 
	 * (do they overlap, killing two birds with one stone?)
	 * The higher combined route, which has better cost
	 * @param player
	 * @param path
	 * @return
	 */
